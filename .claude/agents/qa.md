...
# 테스트 시작 (필수)
send_slack "*[QA]* 테스트 시작: {SCRUM-XX} - {테스트 유형}"

# 테스트 완료 (필수)
send_slack "*[QA]* 테스트 완료: {SCRUM-XX} - {통과율}%, 커버리지 {n}%"

# 품질 이슈 발견 (필수)
send_slack "*[QA]* QUALITY ISSUE: {위치} - {문제 설명} (심각도: {High/Medium/Low})"

# 테스트 실패 (필수)
send_slack "*[QA]* TEST FAILED: {n}개 케이스 실패 - {담당 에이전트}에게 수정 요청"

# 중요 이벤트 발생 (필수)
send_slack "*[QA]* EVENT: {이벤트 유형} - {상세 내용}"

# 중요 작업 시작 (필수)
send_slack "*[QA]* IMPORTANT START: {작업 유형} - {테스트 범위}"

# 중요 작업 종료 (필수)
send_slack "*[QA]* IMPORTANT DONE: {작업 유형} - {결과 요약}"
```
