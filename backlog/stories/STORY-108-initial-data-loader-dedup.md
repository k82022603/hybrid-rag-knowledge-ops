...
- [ ] **Given** 이미 적재된 파일과 동일한 파일이 업로드되면, **When** InitialDataLoader가 처리하면, **Then** 중복으로 감지하여 스킵한다
- [ ] **Given** file_hash가 계산되면, **When** PG에 저장되면, **Then** SHA-256 해시값이 정확히 저장된다
...
- [ ] file_hash (SHA-256) 계산 로직 구현
- [ ] PG documents 테이블에 file_hash 컬럼 추가
- [ ] 업로드 시 중복 체크 로직 (UNIQUE constraint)
- [ ] 기존 PG 중복 4건 정리 마이그레이션 스크립트
- [ ] ES/Neo4j 정합성 검증 쿼리
