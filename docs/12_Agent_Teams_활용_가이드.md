...
## 부록 B: 실전 적용 후기 및 솔직한 평가 (2026-02-07)

> **작성자**: Claude Code (Opus 4.6) | **검증일**: 2026-02-07

### B.1 현실 vs 이상

| 구분 | 이상 (문서의 시나리오) | 현실 (Sprint 08 경험) |
|------|----------------------|----------------------|
| **자율적 협업** | Lead가 지시하면 Teammate가 자율 수행 | 사용자가 직접 작업별 지시 필요 |
| **병렬 실행** | 5명이 동시에 독립 작업 | 실질적으로 1-2명 병렬이 현실적 |
| **역할 분담** | PM→TechLead→Backend 자연스러운 위임 | 메인 클로드가 대부분 직접 수행 |
| **통신 오버헤드** | 메시지 교환으로 협업 조율 | 컨텍스트를 빠르게 소비하는 원인 |

### B.2 Agent Teams가 실제로 잘 되는 것

1. **병렬 코드 분석/리서치**: 서로 다른 파일을 동시에 읽고 분석 (Explore 에이전트)
2. **독립적인 테스트 실행**: 한쪽에서 코드 수정, 다른쪽에서 테스트 돌리기
3. **스탠드업 미팅 시뮬레이션**: 9개 에이전트 메시지 Slack 전송 (자동화 시나리오)

### B.3 Agent Teams가 아직 부족한 것

1. **자율적 위임 체계**: 사용자가 "이 스토리 처리해줘"라고 하면 PM이 알아서 분석 → TechLead에 위임 → Backend 실행하는 체인이 **아직 불가능**
2. **MCP 도구 공유**: Teammate가 Slack MCP, Jira MCP에 직접 접근 불가 (Shell 우회 필요)
3. **파일 동시 수정**: Lock 메커니즘 없음 → 실질적으로 1파일 1에이전트 원칙 필수
4. **컨텍스트 소비**: 에이전트 간 메시지 교환이 컨텍스트를 빠르게 소진
5. **상태 연속성**: 컨텍스트 압축 후 Teammate 상태가 유실될 수 있음

### B.4 현재 프로젝트의 실제 운영 방식

```
[현실적 운영 모델]

사용자 → 메인 클로드 (직접 작업)
           ├── Task(Explore) → 코드 분석 (병렬)
           ├── Task(Explore) → 다른 파일 분석 (병렬)
           ├── 직접 코드 수정
           ├── 직접 테스트 실행 (TEST_MODE=docker)
           ├── MCP Slack → 진행 상황 공유
           └── 직접 Docker 관리

[이상적 Agent Teams 모델 (미달성)]
...
| 개선 영역 | 현재 상태 | 필요한 기능 |
|----------|----------|-----------|
| **자동 위임** | 수동 지시 | "STORY-XXX 처리" → PM이 자동 분배 |
| **MCP 공유** | Shell 우회 | Teammate도 MCP 도구 직접 접근 |
| **파일 Lock** | 없음 | 파일별 배타적 잠금 메커니즘 |
| **컨텍스트 관리** | 빠른 소진 | 에이전트별 독립 컨텍스트 + 요약 교환 |
| **상태 복구** | 유실 가능 | 체크포인트 기반 복구 |

### B.6 결론

> Agent Teams는 **가능성 있는 프레임워크**이지만, 2026-02-07 시점에서는 아직 **실험적 단계**입니다.
>
> 현재로서는 **메인 클로드 + 병렬 서브에이전트(Task 도구)** 조합이 가장 실용적이며,
> Agent Teams의 "자율적 협업"은 아직 사용자의 적극적인 개입 없이는 제대로 작동하지 않습니다.
>
> 이 문서의 시나리오 5.1~5.3은 **목표 비전**으로 이해하시고,
> 실제 운영에서는 B.4의 "현실적 운영 모델"을 기준으로 활용하시기 바랍니다.
