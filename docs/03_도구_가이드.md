...
flowchart TB
    subgraph Tools["Claude Code 도구 계층"]
        ...
        subgraph TopLayer["Top Layer"]
            direction LR
            Agents["Agents<br/>(9개)"]
            Commands["Commands<br/>(24개)"]
            MCP["MCP<br/>(7개)"]
        end

        subgraph AutoLayer["Automation Layer"]
            Scripts["Automation Scripts (4개)<br/>jira-sync, seed-data, worklog"]
        end
...
| **Commands** | 24개 | 작업 자동화 (Hot-Reload 지원) |
| **MCP Servers** | 7개 | 외부 시스템 연동 (Tool Search 포함) |
| **Scripts** | 4개 | 워크플로우 자동화 |
| **Hooks** | 0개 | 구현 예정 (품질 게이트용) |
...
### 4.3 Daily Commands (5개) - `/daily:명령어`
...
### 4.5 Workflows (6개) - `/workflows:명령어`
...
> ⚠️ **현재 Hooks 미구현**: 프로젝트에 아직 Hook 스크립트가 설정되어 있지 않습니다.

### 7.3 구현 예정 Hooks

Sprint 진행에 따라 다음 Hook들이 추가될 예정입니다:

| Hook | 용도 | 예정 Sprint |
|------|------|-------------|
| `pre-commit.sh` | 커밋 전 품질 검사 | Sprint 2 |
| `ragas-gate.sh` | RAG 품질 게이트 | Sprint 3 |
| `quality-gate.sh` | PR 품질 게이트 | Sprint 4 |
| `notification.sh` | Slack 알림 연동 | Sprint 2 |
...
### Commands 카테고리 (3개) ✅
- [x] .claude/commands/daily/ (5개 명령어)
