...
> - **문서 상태**: 현행 (가이드 문서로서 유효, 설정 절차는 그대로 사용 가능)
> - **주요 변경사항**: 실제 연동된 외부 솔루션 현황 — GitHub (사용), Jira Cloud (사용), Slack (MCP + send_slack.sh 혼합 방식 운영), MCP 설정 파일 위치 `.claude/settings.json` (확정). 채널명은 `proj-hrkp-*` 계열로 실 운영됨.
...
```bash
# 필수 정보
JIRA_HOST="your-company.atlassian.net"  # 도메인만 (https:// 제외)
JIRA_PROJECT_KEY="HRKP"  # 프로젝트 키

# 확인 방법: 이슈 URL에서 확인
# https://your-company.atlassian.net/browse/HRKP-123
#         ^^^^^^^^^^^^^^^^^^^^^^^^^^^           ^^^^
#         JIRA_HOST                       JIRA_PROJECT_KEY
```
...
> **[현행화 메모]**: 실제 운영된 Slack 채널 구조는 아래와 같습니다 (채널 ID 포함):
> - `proj-hrkp-dev` (C0A9WGCD733): 개발 작업 기본 채널 (시작/진행/완료/리뷰 통합)
> - `proj-hrkp-standup` (C0A9B7HDEUB): 스탠드업 미팅, 인사
> - `proj-hrkp-alerts` (C0A9WGEVB97): 장애/에러 알림
> - `proj-hrkp-general` (C0AABTM716U): 일반 공지
> 위 문서의 `#hrkp-reviews`는 별도 채널 없이 `proj-hrkp-dev`로 통합 운영됨.
...
> **[현행화 메모]**: 실제 프로젝트에서는 `.claude/settings.json` (프로젝트 로컬)에 MCP 서버 설정을 관리함. `settings.local.json`과 `settings.json` 두 파일 모두 사용됨. Jira 프로젝트 키는 `SCRUM`으로 운영됨 (문서 예시의 `HRKP`와 다름).
...
**주요 변경사항 (v1.1)**:
- Jira: `mcp-server-atlassian` → `@anthropic/mcp-server-jira` (공식 패키지)
- Jira: `JIRA_BASE_URL` → `JIRA_HOST` (도메인만 지정, https:// 제외)
- GitHub: `GITHUB_TOKEN` → `GITHUB_PERSONAL_ACCESS_TOKEN`
- 모든 args에 `-y` 플래그 추가 (자동 설치 확인)
...
- [ ] 환경변수 설정 (`JIRA_HOST`, `JIRA_EMAIL`, `JIRA_API_TOKEN`)
...
- [Claude Code 개발자 통합 가이드](../knowledge_service/docs/05_development/02_developer_integration_guide.md)
...
| 일자 | 작성자 | 내용 |
|------|--------|------|
| 2026-02-20 | Claude (doc-agent) | 프로젝트 종료 후 현행화 — 실제 운영된 Slack 채널 구조 반영, MCP 설정 파일 위치 확정, Jira 프로젝트 키(SCRUM) 명시 |
