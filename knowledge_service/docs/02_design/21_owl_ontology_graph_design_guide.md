...
| **작성자** | 클로드 (Claude Code) |
| **상태** | 초안 (Draft) |
| **상위 문서** | [01_hybrid_rag_platform_detailed_design.md §4.2](./01_hybrid_rag_platform_detailed_design.md) |
| **관련 ADR** | [05_neo4j_subgraph_query_optimization.md](./technical_assessment/05_neo4j_subgraph_query_optimization.md) |
...
| **제외** | RAG 검색 알고리즘 (→ [§6.4 검색 구현](./01_hybrid_rag_platform_detailed_design.md#64-hybrid-검색-구현)), ETL 파이프라인 (→ [ETL 배치 설계서](../03_implementation/etl_batch_pipeline_design.md)) |
...
- [01_hybrid_rag_platform_detailed_design.md §4.2](./01_hybrid_rag_platform_detailed_design.md) — 현행 Neo4j 스키마
- [technical_assessment/05_neo4j_subgraph_query_optimization.md](./technical_assessment/05_neo4j_subgraph_query_optimization.md) — Full-Text Index 전략
- [20_ai_metadata_extraction_design.md](./20_ai_metadata_extraction_design.md) — AI 기반 엔티티 추출
...
현행 Neo4j 스키마([§4.2](./01_hybrid_rag_platform_detailed_design.md#42-neo4j-그래프-스키마))를 OWL로 형식화한 결과입니다.
...
```turtle
@prefix : <http://hrkp.local/ontology#> .
@prefix owl:  <http://www.w3.org/2002/07/owl#> .
...
[`neo4j_storage.py`](../../src/app/storage/neo4j_storage.py)의 `_ENTITY_TYPE_LABEL_MAP`은 LLM 추출 결과를 OWL 클래스로 정규화하는 매핑 테이블입니다.
...
> [CLAUDE.md §인프라 설정 검증 원칙](../../CLAUDE.md)에 따라, 본 가이드에 정의된 모든 제약은 **실동작 검증**까지 완료해야 합니다.
...
[§4.2.3 스키마 진화 Workaround](./01_hybrid_rag_platform_detailed_design.md#423-스키마-진화-workaround)와 동일한 원칙을 OWL에도 적용합니다.
...
자세한 내용은 [05_neo4j_subgraph_query_optimization.md](./technical_assessment/05_neo4j_subgraph_query_optimization.md)를 참고하세요.
...
[`HybridRetriever`](../../src/app/rag/retriever.py)의 Graph 검색 단계에서 온톨로지 기반 확장을 활용합니다.
...
| 문서 | 위치 |
|------|------|
| Hybrid RAG 상세 설계서 v2.4 | [01_hybrid_rag_platform_detailed_design.md](./01_hybrid_rag_platform_detailed_design.md) |
| Neo4j Subgraph 쿼리 최적화 | [technical_assessment/05_neo4j_subgraph_query_optimization.md](./technical_assessment/05_neo4j_subgraph_query_optimization.md) |
| Gleaning 기반 KG 품질 향상 검토 | [technical_assessment/03_gleaning_knowledge_graph_quality_assessment.md](./technical_assessment/03_gleaning_knowledge_graph_quality_assessment.md) |
| AI 메타데이터 추출 설계 | [20_ai_metadata_extraction_design.md](./20_ai_metadata_extraction_design.md) |
| Neo4j 저장 구현 | [`src/app/storage/neo4j_storage.py`](../../src/app/storage/neo4j_storage.py) |
| Hybrid Retriever | [`src/app/rag/retriever.py`](../../src/app/rag/retriever.py) |

### 11.2 외부 자료

| 자료 | URL |
|------|-----|
| 수사 온톨로지 기반 KG 시스템 구축 가이드 (블로그) | https://k82022603.github.io/posts/%EC%88%98%EC%82%AC-%EC%98%A8%ED%86%A8%EB%A1%9C%EC%A7%80-%EA%B8%B0%EB%B0%98-%EC%A7%80%EC%8B%9D-%EA%B7%B8%EB%9E%98%ED%94%84-%EC%8B%9C%EC%8A%A4%ED%85%9C-%EA%B5%AC%EC%B6%95-%EA%B0%80%EC%9D%B4%EB%93%9C/ |
...
> 본 가이드는 Hybrid RAG Knowledge Platform의 Graph 설계 표준 문서입니다.
> 변경 시 [§4.2 Neo4j 그래프 스키마](./01_hybrid_rag_platform_detailed_design.md#42-neo4j-그래프-스키마)와 동기화하고, ETL 파이프라인 영향도를 함께 검토해야 합니다.
