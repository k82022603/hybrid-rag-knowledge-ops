...
> [21_owl_ontology_graph_design_guide.md](./21_owl_ontology_graph_design_guide.md)에서 정의한 OWL 온톨로지를 **신규 WSL2 환경**에 Neo4j로 구축하는 단계별 가이드입니다.
...
| **작성일** | 2026-04-22 |
| **작성자** | 클로드 (Claude Code) |
| **상위 문서** | [21_owl_ontology_graph_design_guide.md](./21_owl_ontology_graph_design_guide.md) |
| **대상 환경** | WSL2 Ubuntu + Docker Desktop (Windows host) |
| **작업 경로** | `/home/ktds/SearcheRAGWithGraphRAG` |
...
```
/home/ktds/SearcheRAGWithGraphRAG/
├── docker/
...
```bash
cat > /home/ktds/SearcheRAGWithGraphRAG/.env <<'EOF'
...
chmod 600 /home/ktds/SearcheRAGWithGraphRAG/.env
```
...
```bash
cat > /home/ktds/SearcheRAGWithGraphRAG/ontology/hrkp-ontology.ttl <<'EOF'
...
ls -la /home/ktds/SearcheRAGWithGraphRAG/ontology/
```
...
```bash
cat > /home/ktds/SearcheRAGWithGraphRAG/scripts/start.sh <<'EOF'
#!/bin/bash
set -e
cd "$(dirname "$0")/../docker"
...
chmod +x /home/ktds/SearcheRAGWithGraphRAG/scripts/start.sh
```

### 6.2 `scripts/stop.sh`

```bash
cat > /home/ktds/SearcheRAGWithGraphRAG/scripts/stop.sh <<'EOF'
#!/bin/bash
cd "$(dirname "$0")/../docker"
docker compose down
...
chmod +x /home/ktds/SearcheRAGWithGraphRAG/scripts/stop.sh
```
...
docker exec -i serag-neo4j cypher-shell -u neo4j -p serag-pass-1234 \
  < /home/ktds/SearcheRAGWithGraphRAG/cypher/02_fulltext.cypher
...
# n10s 초기화
docker exec -i serag-neo4j cypher-shell -u neo4j -p serag-pass-1234 \
  < /home/ktds/SearcheRAGWithGraphRAG/cypher/03_n10s_init.cypher

# OWL Import
docker exec -i serag-neo4j cypher-shell -u neo4j -p serag-pass-1234 \
  < /home/ktds/SearcheRAGWithGraphRAG/cypher/04_owl_import.cypher
...
```bash
docker exec -i serag-neo4j cypher-shell -u neo4j -p serag-pass-1234 \
  < /home/ktds/SearcheRAGWithGraphRAG/cypher/05_sample_data.cypher
...
```bash
cat > /home/ktds/SearcheRAGWithGraphRAG/scripts/bootstrap.sh <<'EOF'
#!/bin/bash
set -e
DC="docker exec -i serag-neo4j cypher-shell -u neo4j -p serag-pass-1234"
ROOT="/home/ktds/SearcheRAGWithGraphRAG"
...
chmod +x /home/ktds/SearcheRAGWithGraphRAG/scripts/bootstrap.sh
...
```bash
cat > /home/ktds/SearcheRAGWithGraphRAG/tests/test_smoke.sh <<'EOF'
#!/bin/bash
set -e
...
chmod +x /home/ktds/SearcheRAGWithGraphRAG/tests/test_smoke.sh
./tests/test_smoke.sh
```
...
```bash
docker exec -i serag-neo4j cypher-shell -u neo4j -p serag-pass-1234 \
  < /home/ktds/SearcheRAGWithGraphRAG/tests/test_consistency.cypher
```
...
python3 -m venv /home/ktds/SearcheRAGWithGraphRAG/.venv
source /home/ktds/SearcheRAGWithGraphRAG/.venv/bin/activate
...
```bash
python /home/ktds/SearcheRAGWithGraphRAG/tests/test_e2e.py
```
...
alias serag-up='cd /home/ktds/SearcheRAGWithGraphRAG && ./scripts/start.sh'
alias serag-down='cd /home/ktds/SearcheRAGWithGraphRAG && ./scripts/stop.sh'
alias serag-test='cd /home/ktds/SearcheRAGWithGraphRAG && ./tests/test_smoke.sh'
...
```bash
cat > /home/ktds/SearcheRAGWithGraphRAG/scripts/healthcheck.sh <<'EOF'
#!/bin/bash
echo "▶ 컨테이너 상태"
docker ps --filter "name=serag-neo4j" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"
...
chmod +x /home/ktds/SearcheRAGWithGraphRAG/scripts/healthcheck.sh
```
...
> 본 가이드는 [21번 OWL 온톨로지 가이드](./21_owl_ontology_graph_design_guide.md)와 짝을 이루는 실행 가이드입니다.
