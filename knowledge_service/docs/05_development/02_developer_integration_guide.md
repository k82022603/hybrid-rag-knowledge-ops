...
### 5.3 서브 에이전트 MCP 제한사항 (중요)

> **2026-01-26 추가**: 서브 에이전트의 MCP 도구 접근 제한에 대한 기술적 설명

#### 5.3.1 문제: 서브 에이전트가 MCP Slack을 사용할 수 없음

MCP 도구(Slack, GitHub, Jira)는 **메인 Claude Code 세션에 바인딩**되어 있습니다.
`Task` 도구로 생성되는 서브 에이전트는 별도 프로세스로 실행되며, 각 에이전트 타입별로 허용된 도구만 사용 가능합니다.

```
┌───────────────────┬───────────────────────────────┬─────────────────┐
│     에이전트      │           허용 도구           │    MCP Slack    │
├───────────────────┼───────────────────────────────┼─────────────────┤
│ qa-engineer       │ Read, Write, Bash, Glob, Grep │ 불가            │
├───────────────────┼───────────────────────────────┼─────────────────┤
│ backend-developer │ All tools (표기상)            │ MCP 연결 미전달 │
├───────────────────┼───────────────────────────────┼─────────────────┤
│ rag-engineer      │ All tools (표기상)            │ MCP 연결 미전달 │
├───────────────────┼───────────────────────────────┼─────────────────┤
│ devops-engineer   │ Bash, Read, Write, Glob       │ 불가            │
├───────────────────┼───────────────────────────────┼─────────────────┤
│ 메인 클로드       │ All tools + MCP 연결          │ ✅ 가능         │
└───────────────────┴───────────────────────────────┴─────────────────┘
```

**기술적 이유**: MCP 서버 연결(Slack, GitHub, Jira)은 메인 Claude Code 세션의 런타임에서 관리되며, 서브 프로세스에는 전달되지 않습니다. 이는 Claude Code의 아키텍처 제약입니다.

#### 5.3.2 해결책: 혼합 방식 (Hybrid Approach)

| 주체 | Slack 전송 방식 | 비고 |
|------|----------------|------|
| **메인 클로드** | MCP Slack (`mcp__slack__slack_post_message`) | 직접 호출 가능 |
| **서브 에이전트** | Shell 스크립트 (`./scripts/send_slack.sh`) | Bash 도구 통해 실행 |

**서브 에이전트용 Shell 스크립트 사용법:**
...
# 예시
./scripts/send_slack.sh dev Backend "작업 완료: AuthController 구현"
./scripts/send_slack.sh dev QA "테스트 결과: 47/47 Pass"
```

**메인 클로드용 MCP Slack 사용법:**

```
mcp__slack__slack_post_message
  channel_id: "C0A9WGCD733"  # dev 채널
  text: "*[클로드]* 작업 시작: STORY-022 JWT Auth Filter"
```

#### 5.3.3 가이드라인

1. **메인 클로드**가 서브 에이전트에 작업을 위임할 때, 서브 에이전트의 프롬프트에 `send_slack.sh` 사용을 명시
2. **메인 클로드**는 작업 시작/완료 알림을 MCP로 직접 전송 (서브 에이전트 대리 가능)
3. **스탠드업 미팅** 등 일괄 메시지는 메인 클로드가 MCP로 직접 전송 (9개 에이전트 메시지를 순차 전송)
...
#### 2026-01-26: 서브 에이전트 MCP 제한사항 발견 및 혼합 방식 도입

| 항목 | 내용 |
|------|------|
| **이슈** | Task 도구로 생성된 서브 에이전트가 MCP Slack에 접근 불가 |
| **원인** | MCP 서버 연결은 메인 세션 런타임에서 관리되며, 서브 프로세스에 미전달 |
| **영향** | 서브 에이전트(Backend, QA, DevOps 등)가 작업 중 Slack 알림 전송 불가 |
| **해결** | 혼합 방식(Hybrid Approach) 도입 - 메인: MCP, 서브: `send_slack.sh` Shell 스크립트 |
| **문서화** | CLAUDE.md v2.18→v2.19 업데이트, 개발자 통합 가이드 섹션 5.3 추가 |

**테스트 결과:**
- 메인 클로드 MCP Slack 전송: ✅ 정상
- 서브 에이전트 Shell 스크립트 전송: ✅ 정상 (`send_slack.sh` v2.0 복원)
- 스탠드업 미팅 (메인 클로드 MCP 대리 전송): ✅ 11/11 성공
