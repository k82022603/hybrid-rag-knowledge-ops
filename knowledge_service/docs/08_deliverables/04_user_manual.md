...
3. [검색 기능](#3-검색-기능)
4. [문서 관리](#4-문서-관리)
5. [Knowledge Graph 탐색](#5-knowledge-graph-탐색)
6. [대화형 검색 (Chat)](#6-대화형-검색-chat)
...
프론트엔드 로그인 화면에서 이메일/비밀번호를 입력합니다. 별도 SSO 연동 없이 AI Service 자체 인증으로 즉시 사용할 수 있습니다.
...
로그인 성공 시 JWT 토큰(HS256)이 발급되며, 이후 모든 API 요청에 `Authorization: Bearer <token>` 헤더로 자동 포함됩니다.
토큰 만료 시 자동으로 갱신(refresh)됩니다.

#### 방법 2: Keycloak SSO 로그인 (OAuth 2.0)

프론트엔드에서 "SSO 로그인" 버튼을 클릭하면 Keycloak 인증 화면으로 이동합니다.
OAuth 2.0 Authorization Code + PKCE 플로우를 사용하며, Keycloak Realm `hybrid-rag`에서 관리됩니다.

| 계정 유형 | ID | 비밀번호 | 권한 |
|----------|-----|---------|------|
| 관리자 | `admin` | `admin123` | 전체 관리 권한 |
| 테스트 사용자 | `test` | `password123` | 검색/조회 권한 |

SSO 로그인 시 RS256 서명의 JWT가 발급되며, Keycloak 토큰 갱신 주기에 따라 자동 갱신됩니다.
...
로그인 후 가장 먼저 표시되는 화면입니다. 시스템 현황을 한눈에 파악할 수 있습니다.

**대시보드 표시 정보**:
- 등록 문서 수, 검색 가능 청크 수
- 시스템 상태 (각 서비스 Health 체크)
- 최근 업로드 문서 목록
- 검색 통계 요약

### 2.2 빠른 검색

대시보드 상단의 검색 입력란에 검색어를 입력하고 검색 버튼을 클릭하면, **검색 페이지로 자동 전환**되면서 해당 검색어로 즉시 검색이 실행됩니다.

```
동작 흐름:
  대시보드 검색 입력란에 "프로젝트 관리" 입력
      |
      v
  검색 버튼 클릭 (또는 Enter)
      |
      v
  /search?q=프로젝트+관리 페이지로 이동
      |
      v
  검색 페이지에서 자동으로 검색 실행 + 결과 표시
```

- URL 형식: `/search?q=검색어`
- 검색 페이지 전환 후 추가 필터 적용 가능
- 브라우저 주소창에 검색 URL을 직접 입력하여 접근하는 것도 가능

---

## 3. 검색 기능

시스템은 프론트엔드에서 **2가지 검색 UI**를 제공합니다.

| 검색 UI | 설명 | 내부 검색 방식 |
|---------|------|-------------|
| **Chat Search** | 자연어 질의 -> DeepSeek V3.2가 답변 합성 + 출처 표시 | 4-Way Hybrid Search |
| **Keyword Search** | 키워드 기반 검색 + 필터링 | BM25 / Vector / Hybrid 선택 가능 |

내부적으로 4가지 검색 엔진(Dense + Sparse + BM25 + Graph)을 **RRF Fusion + Reranker**로 통합하는 **4-Way Hybrid Search**가 가장 높은 정확도를 보입니다.
...
> **참고**: 최대 파일 크기는 2GB(2048MB)입니다. Nginx, Frontend, Docling 모두 1200초(20분) 타임아웃이 적용되어 대용량 파일 업로드 및 처리를 안정적으로 지원합니다.
...
파일 업로드 (MinIO 저장)
    |
    v
Docling 문서 파싱 (PDF/DOCX/HWP -> 텍스트 추출)
    |
    v
청킹 (Semantic Chunking)
    |
    v
임베딩 (BGE-M3, 1024d)
    |
    v
인덱싱 (Elasticsearch + Neo4j)
    |
    v
검색 가능 상태 (completed)
```
...
**대용량 파일 업로드 안내**:
- 최대 **2GB(2048MB)** 파일 업로드 가능
- 대용량 PDF (예: SEC 10-K 보고서 등 수백 페이지)는 처리에 수 분~수십 분 소요될 수 있음
- 타임아웃: Nginx/Frontend/Docling 모두 **1200초(20분)** 적용
- 업로드 중 브라우저를 닫지 않도록 주의

### 4.5 문서 재시도 (Retry)

업로드 후 처리가 실패(failed)한 문서는 **재시도 버튼**을 통해 개별적으로 재처리할 수 있습니다.

| 항목 | 설명 |
|------|------|
| 대상 | 상태가 `failed`인 문서 |
| 위치 | 문서 목록에서 해당 문서의 재시도 버튼 |
| 동작 | 기존 파일을 기반으로 파싱/청킹/임베딩 파이프라인을 다시 실행 |
| 결과 | 성공 시 `completed`, 재실패 시 `failed` 상태 유지 |

**재시도가 필요한 주요 상황**:
- 일시적 서비스 장애로 인한 처리 실패
- Docling 파싱 타임아웃 (대용량 파일)
- 임베딩 모델 일시 장애

---

## 5. Knowledge Graph 탐색
...
- 자연어로 질문을 입력합니다 (예: "프로젝트 관리 방법론은 무엇인가요?").
- 시스템이 4-Way Hybrid Search로 관련 문서를 검색합니다.
- **DeepSeek V3.2** LLM이 검색된 문서를 바탕으로 답변을 합성합니다.
- 답변과 함께 **출처 문서 목록**이 표시되어 근거를 확인할 수 있습니다.
- 출처를 클릭하면 해당 문서의 원문 내용을 확인할 수 있습니다.
