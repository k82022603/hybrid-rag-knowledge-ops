...
### 16.7 임베딩 Full Cycle 배치 OOM Kill 인시던트 (2026-02-10)

#### 사건 요약

| 항목 | 값 |
|------|-----|
| **발생 시각** | 2026-02-10 01:23 KST (추정) |
| **발견 시각** | 2026-02-10 10:06 KST |
| **원인** | WSL2 OOM Killer에 의한 python3 프로세스 강제 종료 |
| **영향** | Full Cycle 배치 프로세스 무응답 종료 (에러 로그 없음) |
| **복구 시각** | 2026-02-10 10:21 KST |

#### 근본 원인

```
dmesg 로그:
oom-kill: task=python3, pid=55680, uid=1000
Out of memory: Killed process 55680 (python3)
  total-vm:8610476kB, anon-rss:2785724kB
```

- 배치 시작 시 메모리: **3.7GB**
- BGE-M3 모델 로드 후: **5.2GB** → **6.8GB** → **7.4GB** (점진 증가)
- 일부 법률 문서 청크(4000자+)의 임베딩 시 메모리 스파이크 발생
- WSL2 가용 메모리 부족 → OOM Killer가 python3 프로세스 선택하여 kill

#### 메모리 증가 추이

```
[시작]    3.7GB → [모델 로드]  5.2GB → [배치 4]  6.8GB
         → [배치 8]  7.1GB → [배치 1324]  7.4GB → [OOM Kill]
```

#### 발견이 늦어진 이유

- 모니터링 스크립트(30분 주기)가 프로세스 상태만 확인
- **에러 로그가 남지 않음** (OOM Kill은 SIGKILL이므로 Python 핸들러가 동작 안 함)
- 배치가 0.2 chunks/s로 느려서 30분 간격 체크로는 중단 감지 지연

#### 복구 조치

1. 배치 프로세스 재시작 (`--max-text-length 1500` 유지)
2. 모니터링 간격 **30분 → 10분**으로 단축
3. 중복 모니터링 프로세스 정리 (3개 → 1개)
...
> **OOM Kill은 SIGKILL이므로 Python의 try/except, signal handler, atexit 모두 동작하지 않는다.**
> 외부 모니터링(별도 프로세스)으로만 감지 가능하며, 모니터링 간격이 곧 최대 다운타임이다.
> 30분 간격 → 최대 30분 무응답 허용이므로, 10분 이하로 줄여야 실용적이다.

---

*작성: Claude Code (Opus 4.6) | 2026-02-10 v3.3*
