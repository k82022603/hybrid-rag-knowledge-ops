...
> **2026-02-09 실측**: CPU 환경에서 `batch_size=16`은 `batch_size=4`보다 **6배 느립니다**.
> sentence-transformers는 CPU에서 텍스트를 순차 처리하므로, 큰 배치 = 긴 단일 연산 시간.
> GPU 환경에서만 큰 batch_size의 병렬화 이점이 있습니다.

| 환경 | 권장 batch_size | 예상 속도 | 비고 |
|------|----------------|----------|------|
| CPU (WSL/Docker) | **4** | ~0.7-0.9 texts/s | 실측 최적값 |
| CPU (네이티브) | 4~8 | ~1.0 texts/s | OS 오버헤드 적음 |
| GPU (CUDA) | 16~32 | ~10+ texts/s | 병렬 처리 이점 |
