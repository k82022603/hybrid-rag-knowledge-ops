...
| Neo4j Entity | **128,000+개** | Phase 3 완료 (Entity/Concept/Technology/Person 등) |
...
1. **Graph RAG 데이터 품질 검증 필요**: Neo4j Entity 128K+ 존재 — 라벨/관계 정합성 검증 및 스키마 통합 필요
...
| Neo4j Entity 수 | 128,000+ | **정합성 검증** | 라벨 누락 수정 + 스키마 v2.6 통합 |
...
flowchart LR
    subgraph Top5["고도화 영역 Top 5"]
        ...
        C["3. Knowledge Graph 정합성<br/>128K Entity 검증 + 스키마 통합"]
...
#### STORY-112: Phase 3 엔티티 추출 정합성 검증 (3 SP)

| 항목 | 내용 |
|------|------|
| **담당** | ETL / RAG |
| **배경** | Neo4j Entity 128,000+ 이미 존재 — 라벨/관계 정합성 검증 필요 |
| **목표** | 기존 엔티티 데이터 품질 검증 + 누락 문서 보완 추출 + PG entity_count 정합 |
| **산출물** | 정합성 검증 리포트, 보완 추출 결과, PG entity_count 일치 확인 |

**완료 기준 (AC)**:
- [ ] Neo4j Entity 128K+ 노드 라벨 분포 검증 (Concept/Technology/Person/Organization 등)
- [ ] RELATED_TO 관계 정상 동작 확인
- [ ] 엔티티 미추출 문서 식별 + 보완 추출
- [ ] PG `entity_count` 필드와 Neo4j 실제 건수 일치 확인
- [ ] 정합성 검증 리포트 작성
