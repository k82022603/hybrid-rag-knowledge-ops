...
파싱 → 청킹 → PG/ES(텍스트)/Neo4j 저장만 수행.
임베딩은 별도 배치로 후처리.
file_hash dedup으로 이미 처리된 파일 건너뜀.
"""
...
# Phase 1: 텍스트 파일만 (파싱 빠름)
# Phase 2: 바이너리 파일 (PDF/PPTX/DOCX/XLSX → Docling OCR 느림)
FAST_EXTENSIONS = {'.md', '.txt', '.json', '.py', '.ipynb', '.ts', '.js',
                   '.java', '.go', '.yaml', '.yml', '.toml', '.xml', '.html',
                   '.css', '.sql', '.sh', '.bat', '.ps1', '.conf', '.csv',
                   '.properties', '.jsx', '.rtf', '.log'}
SLOW_EXTENSIONS = {'.pdf', '.pptx', '.docx', '.xlsx', '.doc'}
...
async def process_file_worker(
    ...
    async with semaphore:
        # Phase 1: 바이너리 파일(PDF/PPTX/DOCX 등)은 건너뜀
        ext = file_info.file_path.suffix.lower()
        if ext in SLOW_EXTENSIONS:
            async with _lock:
                _progress["skipped"] += 1
                _progress.setdefault("deferred_count", 0)
                _progress["deferred_count"] += 1
            return LoadResult(
                file_path=str(file_info.file_path),
                file_name=file_info.file_name,
                status=LoadStatus.SKIPPED,
                error_message=f"Phase 2: {ext} deferred",
            )
        ...
        print(
            f"  [{processed+1}/{total}] {source_name}/{file_info.file_name}",
            flush=True,
        )
...
async def load_source_parallel(
    ...
    tasks = [
        process_file_worker(loader, f, semaphore, total, source.name)
        for f in files
    ]
...
async def main():
    ...
    print("=" * 60)
    print(f"ETL Phase 1 (TEXT ONLY) Started: {started_at}")
    print(f"Workers={NUM_WORKERS}, Embedding=OFF, PDF/PPTX/DOCX=DEFERRED")
    print("=" * 60)
    ...
    loader = InitialDataLoader(
        ...
        enable_embeddings=False,       # 임베딩 OFF
        enable_entity_extraction=False, # 엔티티 OFF
    )
    ...
    print(f"Mode: NO EMBEDDING (parse+chunk+store only)")
    save_progress()

    semaphore = asyncio.Semaphore(NUM_WORKERS)

    print(f"\nStarting fast ETL (semaphore={NUM_WORKERS}, no embedding)...\n")
    ...
    print("\n" + "=" * 60)
    print(f"ETL Pipeline (NO EMBEDDING) Completed!")
    print("=" * 60)
    print(f"  Total:    {total}")
