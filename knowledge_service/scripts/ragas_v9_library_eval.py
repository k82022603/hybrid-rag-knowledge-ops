#!/usr/bin/env python3
"""
RAGAS v9 Library Evaluation - RAGAS 0.2.15 라이브러리 직접 평가
================================================================
LLM-as-Judge가 아닌 RAGAS 라이브러리를 사용하여 v8과 동일한 방법론으로 평가.
v8 (RAGAS 0.4.3)과 완전 동일하진 않지만, Judge 방식보다 정밀한 비교 가능.

실행:
    docker exec kp-ai-service python3 /app/scripts/ragas_v9_library_eval.py
"""
    ...
    try:
        resp = requests.post(
            ...
            json={"model": "deepseek-chat",
                  ...
                  "temperature": 0.1, "max_tokens": 500, "n": 1},
            timeout=60,
        )
...
def run_ragas_library_eval(samples):
    """RAGAS 0.2.15 라이브러리로 평가"""
    ...
    # Build EvaluationDataset with RAGAS 0.2.15 schema
    eval_samples = []
    for s in samples:
    ...
    # Embeddings for answer_relevancy (uses small model for speed)
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  RAGAS v9 Library Evaluation (RAGAS 0.2.15)")
    print(f"  {ts}")
    print(f"{'='*60}\n")

    # 1. Login + Search + Generate
    client = HRKPClient()
    if not client.login():
        sys.exit(1)

    print(f"[Phase 1] Search + Answer ({len(TEST_QUESTIONS)} queries)...")
    samples = []
    for idx, q in enumerate(TEST_QUESTIONS):
        print(f"  Q{idx+1:02d}/{len(TEST_QUESTIONS)} [{q['type']:18s}] {q['question'][:50]}...", end=" ", flush=True)
        t0 = time.time()

        try:
            sr = client.hybrid_search(q["question"], top_k=10)
            contexts = [item.get("content", "") for item in sr.get("results", []) if item.get("content")]
        except Exception as e:
            print(f"FAIL: {e}")
            contexts = []
        ...
        print(f"OK ({elapsed:.1f}s, {len(contexts)} ctx)")

    # 2. RAGAS Library Evaluation
    ...
    try:
        ...
        print(f"\n{'='*60}")
        print(f"  Results (RAGAS 0.2.15 Library)")
        print(f"{'='*60}")
        ...
        for m, v in scores.items():
            ...
            print(f"    {m:22s}: {v:.4f}  (v8: {v8:.3f}, {sign}{delta:.4f})")
        ...
        for idx, row in df.iterrows():
            ...
            s["grade"] = "HIGH" if avg >= 0.70 else ("PARTIAL" if avg >= 0.40 else "NONE")
            per_sample.append(s)
        ...
        for s in per_sample:
            grades[s["grade"]] += 1
        print(f"\n  Quality Gate: HIGH={grades['HIGH']}, PARTIAL={grades['PARTIAL']}, NONE={grades['NONE']}")
        ...
        for d in domains:
            ...
            if ds:
                ...
                print(f"    {d:20s} F={avg_f:.3f} R={avg_r:.3f} P={avg_p:.3f} C={avg_c:.3f}")
        ...
        json_path = "/tmp/ragas_v9_library_result.json"
        with open(json_path, "w", encoding="utf-8") as f:
            json.dump(output, f, ensure_ascii=False, indent=2)
        print(f"\n  JSON saved: {json_path}")

    except Exception as e:
        print(f"\n  [ERROR] RAGAS library evaluation failed: {e}")
        import traceback
        traceback.print_exc()
        eval_elapsed = time.time() - eval_start
