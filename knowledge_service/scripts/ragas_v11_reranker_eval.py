#!/usr/bin/env python3
"""
RAGAS v11 Post-RRF Reranker Evaluation
=======================================
v10 파이프라인에 BGE-Reranker (Post-RRF) 적용 후 품질 변화 측정.
/search/hybrid 엔드포인트 (4-Way RRF + Reranker 파이프라인) 사용.

변경점 (v10 대비):
- Reranker: BGE-Reranker-Base (ONNX) 활성화 - Post-RRF 재정렬
- Pipeline: Dense+Sparse+BM25+Graph -> RRF(k=60) -> Reranker(top_k*2) -> Entity Enrichment
- 나머지 환경 동일 (ES 42,462 chunks, Neo4j 92,209 entities)

실행:
    docker exec kp-ai-service python3 /app/scripts/ragas_v11_reranker_eval.py
"""
...
def main():
    ...
    print(f"\n{'=' * 70}")
    print(f"  RAGAS v11 Post-RRF Reranker Evaluation")
    print(f"  (RAGAS 0.2.15 Library + /search/hybrid + BGE-Reranker)")
    print(f"  {ts}")
    print(f"{'=' * 70}\n")
    ...
    try:
        ...
        # Print results
        print(f"\n{'=' * 70}")
        print(f"  RESULTS: v11(+Reranker) vs v10 vs v9")
        print(f"{'=' * 70}")
        print(f"  {'Metric':22s}  {'v11':>8s}  {'v10':>8s}  {'v9':>8s}  {'v10->v11':>8s}")
        print(f"  {'-' * 60}")
        for m in metric_names:
            v11 = scores[m]
            v10 = V10_BASELINE[m]
            ...
            print(
                f"  {m:22s}  {v11:8.4f}  {v10:8.4f}  {v9:8.4f}  {sign}{delta:.4f}"
            )
        ...
        print(
            f"    (v10: HIGH=30, PARTIAL=11, NONE=10)"
        )
        ...
        output = {
            ...
            "environment": {
                ...
                "reranker_position": "Post-RRF, before Entity Enrichment",
                "endpoint": "/search/hybrid + DeepSeek generate",
                "search_channels": "4-Way RRF (Dense+BM25+Sparse+Graph) + Reranker",
            },
            "metrics": scores,
        ...
        json_path = "/tmp/ragas_v11_reranker_result.json"
    ...
    print(f"\n{'=' * 70}")
    print(f"  v11 Post-RRF Reranker Evaluation Complete!")
    print(f"{'=' * 70}\n")
