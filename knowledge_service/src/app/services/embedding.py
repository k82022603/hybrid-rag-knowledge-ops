...
class EmbeddingService:
    ...
    def embed_batch(
        ...
        # 캐시 배치 조회 (sparse 미사용 시에만)
        cache_results: Optional[List[Optional[List[float]]]] = None
        miss_indices: List[int] = list(range(len(texts)))

        if self._cache and not return_sparse:
            cache_results, miss_indices = self._cache.get_batch(texts, self.model_name)
            if not miss_indices:
                logger.debug(
                ...
                return cache_results  # type: ignore[return-value]
        ...
        # 캐시 저장 (sparse 미사용 시)
        if self._cache and not return_sparse and miss_texts:
            self._cache.set_batch(miss_texts, self.model_name, dense_vectors)

        # 결과 조합 (캐시 히트 + 새 결과)
        if cache_results is not None and not return_sparse:
            final_dense: List[List[float]] = []
            new_idx = 0
            for i in range(len(texts)):
                if cache_results[i] is not None:
                    final_dense.append(cache_results[i])  # type: ignore[arg-type]
                else:
        ...
        cache_hits = len(texts) - len(miss_indices) if cache_results else 0
        logger.info(
            "Batch embed completed: total=%d, cache_hits=%d, computed=%d, "
            "elapsed=%.3fs, rate=%.1f texts/s",
            len(texts),
            cache_hits,
            len(miss_indices),
            elapsed,
