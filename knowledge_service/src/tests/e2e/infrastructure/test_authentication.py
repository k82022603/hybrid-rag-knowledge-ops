...
class TestKeycloakConfiguration:
    ...
    def test_tc_infra_302_realm_exists(
        ...
        """
        TC-INFRA-302: knowledge-platform realm should exist.
        ...
        response = http_session.get(
            "http://localhost:8180/admin/realms/knowledge-platform",
        ...
        data = response.json()
        assert data.get("realm") == "knowledge-platform"
    ...
    def test_tc_infra_303_clients_configured(
        ...
        # Get clients
        response = http_session.get(
            "http://localhost:8180/admin/realms/knowledge-platform/clients",
            headers={"Authorization": f"Bearer {admin_token}"},
            timeout=10
        )
        ...
        expected_clients = ["knowledge-frontend", "knowledge-backend"]
    ...
    def test_tc_infra_304_roles_defined(
        ...
        # Get realm roles
        response = http_session.get(
            "http://localhost:8180/admin/realms/knowledge-platform/roles",
            headers={"Authorization": f"Bearer {admin_token}"},
            timeout=10
        )
...
class TestOAuth2Flow:
    ...
    def test_tc_infra_306_oauth2_password_grant(
        ...
        """
        response = http_session.post(
            "http://localhost:8180/realms/knowledge-platform/protocol/openid-connect/token",
            data={
                "grant_type": "password",
...
class TestUserManagement:
    ...
    def test_tc_infra_305_user_creation(
        ...
        create_response = http_session.post(
            "http://localhost:8180/admin/realms/knowledge-platform/users",
            headers={
        ...
        if create_response.status_code == 201:
            ...
            users_response = http_session.get(
                f"http://localhost:8180/admin/realms/knowledge-platform/users?username={test_username}",
                headers={"Authorization": f"Bearer {admin_token}"},
                timeout=10
            )
            ...
            if users_response.status_code == 200:
                ...
                if users:
                    user_id = users[0]["id"]
                    http_session.delete(
                        f"http://localhost:8180/admin/realms/knowledge-platform/users/{user_id}",
                        headers={"Authorization": f"Bearer {admin_token}"},
                        timeout=10
                    )
