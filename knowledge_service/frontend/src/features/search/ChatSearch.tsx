...
const ChatSearch: React.FC = () => {
  ...
  const handleGraphSourceClick = useCallback((source: Source) => {
    // 그래프 출처이거나 그래프 엔티티 데이터가 있는 소스만 처리
    if (source.sourceType !== 'graph' && !source.graphContext?.relatedEntities?.length) return;

    const entities: string[] = [];

    // graphContext.relatedEntities: 백엔드에서 결과별로 매칭된 엔티티
